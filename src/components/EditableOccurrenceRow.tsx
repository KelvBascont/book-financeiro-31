...
const EditableOccurrenceRow = ({ 
  ...
  const handleSave = async () => {
    ...
    try {
      if (transaction.isRecurringOccurrence && transaction.occurrenceIndex !== undefined) {
        // Para ocorrências recorrentes, vamos criar uma entrada específica
        await onUpdateOccurrence(transaction.id, transaction.occurrenceIndex, newAmount);
      }
      setIsEditing(false);
      toast({
        title: "Sucesso",
  ...
  // Não permitir edição de ocorrências recorrentes por enquanto (apenas o original)
  const canEdit = !transaction.isRecurringOccurrence;
